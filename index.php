...
if ($act == "list") {	// 一覧
  ...
  list($list, $pager, $counter) = item_list($kind, $ord, $page, $limit, $top, $search, $special, 'or');		// こだわりOR検索
	...
	$data["osusume"] = item_list($kind, 0, 0, 3, 2); // ●●●種類ごとのおすすめ
...
if (($act == "detail")||($act == "bukken")) {
	...
	if ($id) {
		...
		if ($item["file1"]) {		// ファイル1
			$item["file1_file"] = Image::getData($item["file1"]);
		}
		if ($item["file2"]) {		// ファイル2
			$item["file2_file"] = Image::getData($item["file2"]);
		}
		if ($item["file3"]) {		// ファイル3
			$item["file3_file"] = Image::getData($item["file3"]);
		}
		if ($item["file4"]) {		// ファイル4
			$item["file4_file"] = Image::getData($item["file4"]);
		}
		if ($item["file5"]) {		// ファイル5
			$item["file5_file"] = Image::getData($item["file5"]);
		}
		$data["item"] = $item;
		// アクセス数
		if (!$_SESSION["INFO"][$id]) {
		...
		//
		$data["osusume"] = item_list($item["kind"], 0, 0, 3, 2);	// ●●●種類ごとのおすすめ
		if (!$html) {
...
if (($act == "toiawase")||($act == "toiawase_reinput")) {
	...
	if ($mode == "form") {
		...
		$tmp = $pre_admin . "\n" . $body . "\n\n" . $post_admin;
		if ($admin_mail) {
			sendmail2($from_mail, $admin_mail, $subject, $tmp, null, $from_name);
		}
		if ($form["email"]) {
...
// --------------------------------
// トップページ
$data["osusume"] = item_list(0, 0, 0, 3, 2);	// おすすめ
//
$data["list0"] = item_list(0, 0, 0, 6, 1);	// 全種類
$data["list1"] = item_list(1, 0, 0, 4, 1);	// 種類1
$data["list2"] = item_list(2, 0, 0, 4, 1);	// 種類2
$data["list3"] = item_list(3, 0, 0, 4, 1);	// 種類3
$data["list4"] = item_list(4, 0, 0, 2, 1);	// 種類4
//
